...
import sys
sys.path.insert(0, "/home/hadoop-hmart-waimai-rank")
...
class TestW8A8Int8FusedMoE(unittest.TestCase):
    ...
    def _w4a8_int8_fused_moe(self, M, N, K, E, topk, block_size, dtype, seed, num_bits):
        ...
        w1 = (w1_fp16 * int8_max).clamp(min=int8_min, max=int8_max).to(torch.int8)
        w2_fp16 = (torch.rand((E, K, N), dtype=dtype) - 0.5) * 2
        w2 = (w2_fp16 * int8_max).clamp(min=int8_min, max=int8_max).to(torch.int8)

        # Generate scale for each column (per-column quantization)
        w1_s = torch.rand(E, 2 * N, device=w1_fp16.device) * factor_for_scale
        w2_s = torch.rand(E, K, device=w2_fp16.device) * factor_for_scale
