...
def _w4a8_block_int8_matmul(
    ...
    # Process 4-bit weights
    offs_bn_packed = offs_bn // 2  # 2 elements per packed byte
    b_ptrs = B + (offs_k[:, None] * stride_bk + offs_bn_packed[None, :] * stride_bn)
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        b_packed = tl.load(
            b_ptrs,
            mask=(offs_k[:, None] < K - k * BLOCK_SIZE_K) & (offs_bn_packed[None, :] < (N // 2)),
            other=0
        )
        ...
        # Reconstruct full int8 matrix
        n_indices = tl.arange(0, BLOCK_SIZE_N)
        n_packed = n_indices // 2
        is_even = (n_indices % 2) == 0
        high_values = high_bits[:, n_packed]
        low_values = low_bits[:, n_packed]
        b = tl.where(is_even, high_values, low_values)
